...
$query = "select * from subarea order by id asc";
$res = $con->query($query);
$count = $res->num_rows;
...
//need to change for another table
$query = "select * from subarea order by id asc limit ".$con->real_escape_string($start).",$recordsPerPage";
//echo $query;exit;
$res    =   $con->query($query);
$count  =   $res->num_rows;
...
if($count > 0)
{
    ...
    {
        $HTML .= '<tr><td><span id="editsubareaid">'.$row['id'].'</span></td>';
		$HTML .= '<td><span id="editsubareaname">'.$row['subareaname'].'</span></td>';
		$HTML .= '<td><span id="editareaid">'.$row['areaid'].'</span></td>';
		$HTML .= '<td><span id="editcreatedate">'.$row['createdate'].'</span></td>';
